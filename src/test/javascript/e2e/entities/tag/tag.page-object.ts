...
export class TagUpdatePage {
  ...
  userStorySelect = element(by.id('field_userStory'));
  ...
  async userStorySelectLastOption(timeout?: number) {
    await this.userStorySelect
      .all(by.tagName('option'))
      .last()
      .click();
  }

  async userStorySelectOption(option) {
    await this.userStorySelect.sendKeys(option);
  }

  getUserStorySelect(): ElementFinder {
    return this.userStorySelect;
  }

  async getUserStorySelectedOption() {
    return await this.userStorySelect.element(by.css('option:checked')).getText();
  }
