...
describe('Tag e2e test', () => {
  ...
  it('should load Tags', async () => {
    ...
    expect(await tagComponentsPage.getTitle()).to.eq('rbRecapApp.tag.home.title');
  });

  it('should load create Tag page', async () => {
    await tagComponentsPage.clickOnCreateButton();
    tagUpdatePage = new TagUpdatePage();
    expect(await tagUpdatePage.getPageTitle()).to.eq('rbRecapApp.tag.home.createOrEditLabel');
  ...
  it('should create and save Tags', async () => {
    ...
    await tagComponentsPage.clickOnCreateButton();
    await promise.all([tagUpdatePage.setKeyInput('key'), tagUpdatePage.setNameInput('name'), tagUpdatePage.userStorySelectLastOption()]);
    expect(await tagUpdatePage.getKeyInput()).to.eq('key', 'Expected Key value to be equals to key');
  ...
  it('should delete last Tag', async () => {
    ...
    expect(await tagDeleteDialog.getDialogTitle()).to.eq('rbRecapApp.tag.delete.question');
